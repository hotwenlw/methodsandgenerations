...
(function() {
    ...
    var or,
        ...
        config = {
            container: 'circle',
            colors: {
                '30': '#0099CE', '40': '#FFA700', '50': '#1FA12D', '61' : '#00BFFF',
                '62': '#E2001A', '70': '#FFD700', '80': '#AB0C9F'
            }, strokes: {
                '30': '#0099CE', '40': '#FFA700', '50': '#1FA12D', '61' : '#00BFFF',
                '62': '#E2001A', '70': '#FFD700', '80': '#AB0C9F'
            }
        ...
        monthNamesAbbr = ["Jan","Feb","March","April","May","June","July","Aug","Sep","Oct","Nov","Dec"],
        lines = [],
        party_arcs = {},
        party_labels = {},
        donor_labels = {},
    ...
    function drawDonationCircle() {
        ...
        $.each(data.donations, function(i, don) {
            ...
            var p = paper.path('M'+dx+' '+dy+' C'+dxc+','+dyc+' '+pxc+','+pyc+' '+px+','+py)
                .attr({
                    stroke: config.strokes[don.party] || config.colors[don.party],
                    opacity: 0.5,
    ...
    function updateFilter() {
        ...
        $.each(lines, function(i, line) {
            ...
            if (visible) {
                donation_donors[d.key] = true;
                donation_parties[d.party] = true;
            }
            line.opacity(visible ? 0.5 : 0.02);
        });

        $.each(donor_labels, function(key, t) {
            var visible = donor_visible(key) && donation_donors[key];
            t.opacity(visible ? 1 : 0.2);
            if (visible) visible_donors.push(key);
        });
        $.each(party_arcs, function(key, arc) {
            var visible = party_visible(key) && donation_parties[key];
        ...
        if (true) {
            var title = visible_parties.length == data.num_parties && visible_donors.length == data.num_donors ? 'Calender' :
    ...
    function fmt_amount(val, plain) {
        ...
        var sp = !plain ? '&nbsp;' : ' ';
        if (val >= 1000) {
            return (val / 1000).toFixed(2).replace('.', ',') + sp+'M';
        }
        return val + '.000'+sp;
    }
